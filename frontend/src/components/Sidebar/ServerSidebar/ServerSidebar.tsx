...
import convert2base62 from '../../../utils/convert2base64';
...
const ServerSidebar = () => {
  ...
  const [servers, setServers] = useState<ServerProps[]>(initialServer);
  ...
  useEffect(() => {
    convert2base62(watch('img')[0]).then(res => {
      setPreviewAvatar(res);
    });
  }, [watch('img')[0]]);
  ...
  const onSubmitHandler: SubmitHandler<FieldValues> = data => {
    if (watch('img')[0]) {
      convert2base62(watch('img')[0]).then(res => {
        serverApi
          .createNewServer({ name: data.name, img: res })
          .then(res => {
            if (res.status === 200) {
  ...
  return (
    <>
      {servers.map(server => (
        <Tooltip
          title={`${server?.name.charAt(0).toUpperCase()}${server?.name.slice(1)}'s Server`}
          arrow
          placement="right"
          key={server?._id}
        >
        ...
        hover:opacity-80 duration-300 transition-all"
          >
            <ServerItem _id={server?._id} imgUrl={server?.imgUrl} name={server?.name} />
          </div>
        </Tooltip>
      ))}
      <Tooltip title="Add New Server" arrow placement="right">
        <div
          className="bg-[#5865f2] text-white m-0 p-0 mt-[10px] rounded-2xl w-12 h-12 min-w-0 flex items-center justify-center cursor-pointer
        hover:opacity-80 duration-300 transition-all text-2xl"
        ...
        <Wrapper onSubmit={handleSubmit(onSubmitHandler)}>
          <div className="px-6 my-6">
            ...
            <Typography
              ...
              sx={{ mt: 1 }}
              className="text-center text-mainColor"
            >
                ...
                {previewAvatar ? (
                  ...
                  <>
                    <input
                      ...
                      type="file"
                      {...register('img')}
                      className="opacity-0 w-20 h-20 absolute top-[50%] left-[50%] cursor-pointer -translate-x-[50%] -translate-y-[50%]"
                    />
